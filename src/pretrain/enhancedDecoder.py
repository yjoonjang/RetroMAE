...
from transformers.models.modernbert.modeling_modernbert import ModernBertMLP, ModernBertRotaryEmbedding
...
def sdpa_attention_forward(
	module: "ModernBERTCrossAttention",
	q: torch.Tensor,
	kv: torch.Tensor,
	...
	**_kwargs,
	...
	query = q.view(bs, q_len, module.num_heads, module.head_dim).transpose(1, 2)
	...
	attn_output = attn_output.view(bs, q_len, module.all_head_size)
...
class ModernBERTCrossAttention(nn.Module):
	def __init__(self, config):
		...
		self.Wkv = nn.Linear(config.hidden_size, 2 * self.all_head_size, bias=config.attention_bias)
	...
	def forward(
		...
		attention_mask: Optional[torch.Tensor], # Position-specific mask
		position_ids: Optional[torch.LongTensor],
	) -> tuple[torch.Tensor]:

		bs, q_len, _ = query_states.shape
		_, kv_len, _ = key_value_states.shape
		
		q = self.Wq(query_states)
		kv = self.Wkv(key_value_states)

		attn_outputs = MODERNBERT_ATTENTION_FUNCTION[self._attn_implementation](
			self,
			q=q,
			kv=kv,
...
class ModernBertLayerForDecoder(nn.Module):
	def __init__(self, config):
		super().__init__()
		self.attn = ModernBERTCrossAttention(config)
	...
	def forward(
		...
		attention_outputs = self.attn(
			...
			self.attn_norm(key_value_states),
			attention_mask,
			position_ids,
		)
		attention_output = attention_outputs[0]
		hidden_states = query_states + attention_output
